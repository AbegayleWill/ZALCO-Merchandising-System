  ...
  private function validateCompName(){
    ...
    if(empty($val)){
      ...
      if(!preg_match("/^[a-zA-Z\s]+$/", $val)){
        $this->addError('compName','Competitor Name field must be letters');
      }
    }
  ...
  private function validateBrandName(){
    ...
    if(empty($val)){
      $this->addError('brandName', 'Brand Name field cannot be empty');
        ...
        if(!preg_match("/^[a-zA-Z\s]+$/", $val)){
            $this->addError('brandName','Brand Name field must be letters');
        }
    }
  ...
  private function validateComActivity(){
    ...
    if(empty($val)){
      $this->addError('notes', 'Competitor Activity field cannot be empty');
        ...
        if(!preg_match("/^[a-zA-Z\s]+$/", $val)){
            $this->addError('notes','Competitor Activity field must be letters');
        }
    }
